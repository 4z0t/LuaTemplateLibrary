...
namespace Lua
{
	...
	public:
		...
		static int Function(lua_State* l)
		{
			...
			using Indexes = std::index_sequence_for<TArgs...>;
			static_assert(std::is_invocable<FnType, TUpvalues&..., TArgs...>::value, "Given function can't be called with such arguments!");
			...
			ArgsTupleT args;
			GetArgs<0, ArgsTupleT, TArgs ...>(l, args);
			if constexpr (std::is_void<TReturn<TArgs...>>::value)
			{
				CClosure::CallHelper(upvalues, args, std::index_sequence_for<TUpvalues...>{}, Indexes{});
				ReplaceUpvalues(l, upvalues);
				return 0;
			}
			else
			{
				TReturn<TArgs...> result = CClosure::CallHelper(upvalues, args, std::index_sequence_for<TUpvalues...>{}, Indexes{});
				ReplaceUpvalues(l, upvalues);
				size_t n_results = PushResult(l, result);
				return n_results;
			}
